...
## ==== Insight tab data prep start ==

it1 <- readRDS("www/data/data_insighttab_1.rds")

df_longNEW22 <- it1$hm
percent_all_NEW_C <- it1$bp
...
## Insight tab 2
it2 <- readRDS("www/data/data_insighttab_2.rds")

df_long2PC_CM <- it2$hm
stacked_dataPC_MP_C <- it2$bp


#colors for heatmap according to answer
...
gg_hm_it2 <- it2$hm |> 
  mutate(tooltip = glue("In <b>{Country}")) |>  #</b>, the national suveillance
  #for <b>{} <i>{xlab}</i> </b> in <b>{type}</b>
  #{}")) |> 
  ...
  scale_fill_manual(name = "Population coverage",
                    ...
                    breaks = c("76-100%", "51-75%", "26-50%","1-25%", "Not part of national surveillance","Do not know"),
                    labels = c("76-100%", "51-75%", "26-50%","1-25%", "Not part of national surveillance","Do not know")) +
...
## Insight tab 2_2
it2_2 <- readRDS("www/data/data_insighttab_2_2.rds")

df_long2GR <- it2_2$hm
stacked_dataGR_NEW_C <- it2_2$bp


#colors for heatmap according to answer
surv_colorsGR <- c("HIGH: all main geographical regions of the country are covered." = "#002D89",
...
gg_hm_it2_2 <- 
  ...
  scale_fill_manual(name = "Geographical/\nrepresentativeness",
                    ...
                    breaks = c("HIGH: all main geographical regions of the country are covered.", "MEDIUM: most geographical regions of the country are covered.", "LOW: a few geographical areas of the country are covered.", "Not part of national surveillance","Do not know"),
                    labels = c("HIGH: all regions", "MEDIUM: most regions", "LOW: a few regions","Not part of national surveillance","Do not know")) +
  labs(x = "culture material and pathogen", y = "Country",
...
## Insight tab 3
it3 <- readRDS("www/data/data_insighttab_3.rds")

df_long2GR <- it3$hm
#stacked_dataGR_NEW_C <- it3$bp
...
gg_hm_it3 <- 
  ...
  geom_tile_interactive(aes(tooltip=tooltip),
                        color = "white", linewidth = 0.5) +
  scale_fill_manual(name = "National guidance in place",# data at national level",
...
ui <- shinyUI(fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      # Sections filter - moved to sidebar
      tags$div(
          ...
          navset_card_underline(
            ...
            nav_panel("Insight #1",
                      page_fillable(
                        ...
                        layout_columns(
                          card(
                            actionButton("reset_it1", label = "Reset selection"),
                        ...
                        card("Selected countrie(s) head-to-head comparison:",
                             tableOutput("country_context_if1")
                        ),
                        col_widths = c(5, 7, 12),
                        row_heights = c(5,4)
                        )),
             ...
             nav_panel("Insight #2",
                       page_fillable(
                         ...
                         layout_columns(
                           card(
                             actionButton("reset_it2", label = "Reset selection"),
                             girafeOutput("plot_it2", height = "100%"),
                           ),
                           card(
                         ...
                         card("Selected countrie(s) head-to-head comparison:",
                              tableOutput("country_context_if2")
                         ),
                         col_widths = c(5, 7, 12),
                         row_heights = c(5,4)
                       )
             ...
             nav_panel("Insight #3",
                       page_fillable(
                         ...
                         layout_columns(
                           card(
                             actionButton("reset_it3", label = "Reset selection"),
                             girafeOutput("plot_it3", height = "100%"),
                           ),
                           card(
...
server <- function(input, output, session) {
  ...
  ## Insight tab related stuff
  
  ### ===== Insight tab 1
  
  ## select country via clic
 # output$hover_selected_it1 <- renderPrint({
#    input$plot_selected
#  })
  ...
  observeEvent(input$reset_it1, {
    session$sendCustomMessage(type = 'plot_it1_set', message = character(0))
  })
  
  # Update selectInput when a country is clicked in the plot
  observeEvent(input$combined_selection, {
    ...
    if (length(selected) > 0) {
      updateSelectInput(
        session,
        "selected_country_it1",
        selected = input$hover_selected_it1#selected
      )
    }
  })
  ...
  output$country_context_if1 <- renderTable({
    ...
    it1$hm %>% # Count per country and response category
      group_by(Country, value) %>%
      summarise(Number = n(), .groups = "drop") %>%
      # Ensure all three categories appear for each country
      complete(
        Country,
...
#  output$md_content_it1 <- renderUI({
 #   HTML(readLines("www/md/tab1.html"))
#  }) 
  ...
  ## select country via clic
  #output$hover_selected_it2 <- renderPrint({
  #  input$plot_it2_selected
  #})
  ...
  observeEvent(input$reset_it2, {
    session$sendCustomMessage(type = 'plot_it2_set', message = character(0))
  })
  ...
  observeEvent(input$combined_selection2, {
    ...
    if (length(selected) > 0) {
      updateSelectInput(
        session,
        "selected_country_it2",
        selected = input$hover_selected_it2#selected
      )
    }
  })
  ...
  output$country_context_it2 <- renderTable({
    ...
    #out <- 
    it2$hm %>% # Count per country and response category
      group_by(Country, value) %>%
      summarise(Number = n(), .groups = "drop") %>%
      # Ensure all three categories appear for each country
      complete(
  ...
  observeEvent(input$reset_it2_2, {
    session$sendCustomMessage(type = 'plot_it2_2_set', message = character(0))
  })
  ...
  observeEvent(input$combined_selection, {
    ...
    if (length(selected) > 0) {
      updateSelectInput(
        session,
        "selected_country_it2",
        selected = input$hover_selected_it2_2#selected
      )
    }
  })
  ...
  output$country_context_it2_2 <- renderTable({
    ...
    #out <- 
    it2_2$hm %>% # Count per country and response category
      group_by(Country, value) %>%
      summarise(Number = n(), .groups = "drop") %>%
      # Ensure all three categories appear for each country
      complete(
  ...
  observeEvent(input$reset_it3, {
    session$sendCustomMessage(type = 'plot_it3_set', message = character(0))
  })
  ...
  observeEvent(input$combined_selection, {
    ...
    if (length(selected) > 0) {
      updateSelectInput(
        session,
        "selected_country_it3",
        selected = input$hover_selected_it3#selected
      )
    }
  })
  ...
  output$country_context_it3 <- renderTable({
    ...
    #out <- 
    it3$hm %>% # Count per country and response category
      group_by(Country, value) %>%
      summarise(Number = n(), .groups = "drop") %>%
      # Ensure all three categories appear for each country
      complete(
